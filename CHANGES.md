...
## 2026/05/14 : 2.19.2 - 20260514

### Fix

* VRMAnimationのエクスポートに、視線移動を含められるように修正。


## 2026/03/31 : 2.19.1 - 20260331

### Fix

* VRoidHubのOAuth認証仕様変更に伴い、PKCEに対応。

## 2025/12/27 : 2.19.0 - 20251227

### New/Update

* MediaPipeのバージョンを最新に更新。
* MediaPipeのウィンドウ内容を刷新。
* 簡単IKモードの各コンボボックスをオートコンプリートに対応。

## Fix

* モバイル端末のブラウザでWebAR表示したときに、タッチと実際の画面の判定がズレる問題を修正。
* カメラのレンダーテクスチャのUIと実際の反映がずれて分かりづらかったのを修正。
* ゲームパッドで操作対象を切り替えた時、メインカメラまたは現在オブジェクトを正常に操作できなかったのを修正。
* Stageの照明の色を調整。

## 2025/07/10 : 2.18.0 - 20250710

### New/Update


* vvmmot関係
    - アプリ用モーションファイル(.vvmmot)にサムネイル情報を追加。
    - アプリ用モーションの保存時にサムネイル情報も保存するように対応。
    - 拡張機能のGoogleAppsScriptもサムネイル情報に対応のため更新。(別途GoogleAppsScriptの再デプロイが必要です)
* VRMAnimation
    - Expressionも含められるように対応。
    - LookAtを.vrma中に含められるように一応対応。（実際は再生側での対応が必要になる想定）
    - リボンバーの履歴にVRMAnimationを追加。
* ポーズ・モーションダイアログ
    - 新たにパネル表示も追加。（タイムラインパネルと切り替え式です）
    - Googleドライブ、アプリサーバを読み込んだときにキャッシュをするように仕様追加。アプリ終了時に一時データは削除されます）
* IKマーカー
    - LeftHand, RightHandのIKマーカーを手のひらに近い形に変更。

### Fix

* ARモードで操作したIKマーカーがたまに吹っ飛んでしまう問題を今度こそ再修正。
* VRMAnimationで最初のモーションに最後のモーションが混じってしまうのを修正。


## 2025/07/02 : 2.17.2 - 20250702

### New/Update

### Fix

* プロジェクトファイル・モーションファイル（vvmmot）を読み込んだときに、VRMのオブジェクト装着のデータのうち、Hipsの装備データが復元されなかったのを修正。
* ARモードで操作したIKマーカーがたまに吹っ飛んでしまう問題を修正。


## 2025/05/11 : 2.17.1 - 20250511

### New/Update

* UniVRM v129にアップデート。
* VRMの自動まばたきをデフォルトでは無効に変更。

### Fix

* ARモード
    - ARモードに切り替わった直後にVRMなどのオブジェクトがタッチされたという誤判定により、ポーズが崩れることがあったのを、可能な限り修正。（完璧ではありません。安全策として必ずキーフレームに登録してからARモードに切り替えることを強く推奨します）。
* VRMの自動瞬きが機能しなくなっていたのを修正。
* VRMのスプリングボーンが激しく動き回ることがあったのを修正。
...
* ARモード
    - モバイル端末にてIKマーカー2点タッチで回転させるように対応。
...
* Stageオブジェクトの照明の影の強さの初期値を1に変更。
* EyeViewHandle（目）のIKマーカーを裏側にも色を描写（分かりづらかったので）
...
* 通常画面とVR/ARモードのカメラ位置が同じになってしまっていたので元通り、別々の位置・回転の管理に戻した。
